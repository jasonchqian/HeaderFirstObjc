...
- (void)connectionDidFinishLoading:(NSURLConnection *)theConnection
{
	...
	NSString    *docDirPath     = ((docDirPaths && docDirPaths.count) > 0) ? [docDirPaths objectAtIndex:0] : nil;
	NSString    *savePath       = (docDirPath) ? [docDirPath stringByAppendingPathComponent:@"weather.xml"] : nil;
    
    if (savePath) {
        [_dataReceived writeToFile:savePath
                        atomically:YES];
    }
